...
- CUA is not selected by default until its node-local provider is installed,
  compatible, enabled, permission-ready, and policy-ready.
...
OC-2 + OC-4 + CUA-2/3 --------------------------> OC-5 CUA plugin
OC-3 + OC-4 + CUA-1/3 --------------------------> OC-6 Linux host
OC-5 + OC-6 ------------------------------------> OC-7 Linux gate

OC-7 + CUA-1 ----------------+-------------------> OC-8 macOS
OC-4/5 + CUA-1 --------------+-------------------> WIN-1 Windows
...
`OC-1`, `CUA-1`, `CUA-2`, and `CUA-3` are the initial parallel lanes. `OC-2`
and `OC-3` can be developed in parallel once the `OC-1` wire fixtures are
stable, although both merge after `OC-1`. `OC-5` and `OC-6` can then be
...
| Invocation | `computer.provider.call.v1` carries one selected node, provider, generation, tool, classified arguments, session, and deadline. |
| Inventory | Readiness, driver provenance, authorization, tool catalog, skill digest, and permission attribution use closed states. |
| Errors | Unknown provider/tool, stale generation, host busy, local approval required, rebound/retry required, and backend unavailable have stable codes. |
| Results | Existing MCP text, image, structured result, cancellation, timeout, and size-limit behavior is preserved. |
...
| Session binding | One v1 host lease maps to one node-injected opaque CUA session; callers cannot supply the native session id. |
...
- Repo: `trycua/cua`
- Depends on: none
- Can run with: `CUA-1`, `CUA-3`, `OC-1`
- Work:
  - Publish a versioned OpenClaw skill profile that retains CUA workflow,
    fallback, browser, snapshot, and verification guidance while removing
    direct CLI/bootstrap instructions.
...
- Work:
  ...
  - Carry provider-spec digest, generation, tool catalog, policy state, skill
    state, and closed readiness codes.
...
- Can run with: `OC-3`
- Work:
  - Add the prepared host/provider/session binding and deterministic selected
...
- Proof:
  ...
  - A test provider returns text, image, and structured results through the
    classified route and cleans up on disconnect or cancellation.
...
- Repo: `openclaw/openclaw`
- Depends on: `OC-2`, `OC-4`, `CUA-2`, relevant `CUA-3` contracts
- Can run with: `OC-6`
- Work:
  ...
  - Inject opaque run-scoped CUA session ids and reject caller-supplied native
    session fields.
...
- Repo: `openclaw/openclaw`
- Depends on: `OC-1`, `OC-3`, `OC-4`, `CUA-1`, relevant `CUA-3` contracts
- Can run with: `OC-5`
- Work:
...
- Work:
  ...
  - Exercise snapshot/action/verification, browser binding, recording/resource
    restrictions, stale generation, Stop, and cleanup.
- Proof:
...
- Repo: `openclaw/openclaw`
- Depends on: `OC-4`, `OC-5`, `OC-7`, `CUA-1`
...
- Work:
  ...
  - Add local permission, restart, crash recovery, exact-resource approval,
    rebound/retry, persistent indicator, and Stop flows.
...
- Proof:
  ...
  - Revocation, restart, stale references, and Stop fail closed.

### WIN-1: Windows companion provider host

- Repo: `openclaw/openclaw-windows-node`
- Depends on: `OC-1`, `OC-4`, `OC-5`, `CUA-1`
...
- Work:
  ...
  - Own managed artifact lifecycle, inherited IPC, dynamic inventory, bounded
    policy, local approval, indicator, Stop, update, and rollback.
...
- Work:
  - Complete hostile-argument, stale capability, policy composition, protected
    consent, host-busy, recording isolation, and arbitrary-exec conflict tests.
...
- Work:
  - Exercise clean install, compatible system import, managed install, update,
    ...
    for macOS and Linux.
  - Verify child cleanup, input release, generation changes, and previous
    known-good recovery after crashes.
...
- Work:
  ...
  - Verify snapshot-before/after, element invalidation, fallback ladders,
    foreground transitions, and explicit DOM-event behavior.
...
| OC-5 | Bundled CUA plugin | OC-2, OC-4, CUA-2/3 | Plugin integration |
| OC-6 | Linux provider lifecycle | OC-3/4, CUA-1/3 | Linux node |
| OC-7 | Linux integration gate | OC-5/6 | End-to-end proof |
| OC-8 | macOS embedded host | OC-7, CUA-1 | Native macOS app |
| WIN-1 | Windows embedded host | OC-4/5, CUA-1 | Windows companion |
...
```text
CUA-1 -> OC-6 -> OC-7 -> OC-8/WIN-1 -> OC-10A-C/WIN-2 -> OC-11
```
...
ship dependency. The RFC should not weaken local IPC isolation merely to make
the current path-addressed embedded endpoint fit the schedule.
