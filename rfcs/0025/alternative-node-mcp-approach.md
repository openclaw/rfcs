...
> **Alternative retained for comparison.** This is the original node-hosted
> native MCP proposal from RFC 0025. The primary RFC now standardizes the
> existing `screen.snapshot` + `computer.act` architecture instead. Its former
...
The detailed [implementation plan](alternative-node-mcp-implementation-plan.md) organizes work
